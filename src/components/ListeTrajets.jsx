    ...
    <div style={{ position: "relative", minHeight: "100vh", background: "#0a1a0f", padding: "3rem 2rem", fontFamily: "Outfit,sans-serif" }}>
      ...
      <style>{`
        ...
        .trajet-card::before {
          ...
          background: linear-gradient(90deg, #22c55e, transparent); opacity: 0; transition: opacity 0.3s ease;
        }
        .trajet-card:hover {
        ...
        .btn-primary {
          ...
          border: none; border-radius: 12px; cursor: pointer; box-shadow: 0 4px 15px rgba(34,197,94,.25);
          transition: all 0.2s ease; flex: 1; text-align: center;
        }
        .btn-primary:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(34,197,94,.4); }
        .btn-primary:disabled { background: rgba(255,255,255,.06); color: rgba(255,255,255,.3); box-shadow: none; cursor: not-allowed; border: 0.5px solid rgba(255,255,255,.1); }

        .btn-secondary {
          padding: 10px 18px; background: rgba(255,255,255,.03);
          color: #fff; font-family: 'Outfit', sans-serif; font-size: 13px; font-weight: 700;
      ...
      <div style={{ position: "relative", zIndex: 10, maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "20px", marginBottom: "30px" }}>
          <div style={{ animation: "fadeIn 0.6s ease forwards" }}>
            ...
            <p style={{ fontSize: 15, color: "rgba(255,255,255,.5)", marginTop: 8, fontWeight: 500 }}>Trouvez votre covoiturage pour l'EMSI aujourd'hui.</p>
          </div>
        ...
        {/* LOADING & ERROR */}
        {loading && <p style={{ color: "#4ade80", textAlign: "center", fontSize: 18, fontWeight: 700, animation: "fadeIn 1s infinite alternate" }}>Chargement des trajets... ⏳</p>}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "24px" }}>
          {trajetsFiltres.length === 0 && !loading && !erreur ? (
            <div style={{ gridColumn: "1 / -1", textAlign: "center", padding: "60px 0", background: "rgba(255,255,255,.02)", borderRadius: 30, border: "1px dashed rgba(255,255,255,.1)", animation: "fadeIn 0.6s ease forwards" }}>
              <div style={{ fontSize: 50, marginBottom: 16, opacity: 0.5 }}>🛣️</div>
              <p style={{ color: "rgba(255,255,255,.5)", fontSize: 18, fontWeight: 500 }}>Aucun trajet trouvé avec ces critères 😕</p>
            </div>
          ) : (
                ...
                <div key={trajet.id} className="trajet-card" style={{ animationDelay: `${index * 0.1}s` }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ background: "rgba(255,255,255,.03)", padding: "6px 14px", borderRadius: 12, border: "1px solid rgba(255,255,255,.05)" }}>
                      <p style={{ fontSize: 22, fontWeight: 900, color: "#4ade80", margin: 0 }}>
                        {trajet.prixParPlace} <span style={{ fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,.3)" }}>MAD</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div style={{ position: "relative", paddingLeft: "30px", display: "flex", flexDirection: "column", gap: "20px", marginTop: "10px" }}>
                    ...
                    <div style={{ position: "relative" }}>
                      <div style={{ position: "absolute", left: "-31px", top: "2px", width: "18px", height: "18px", borderRadius: "50%", background: "rgba(8,22,12,1)", border: "4px solid #4ade80", zIndex: 2 }}></div>
                      <p style={{ margin: 0, fontSize: 12, color: "rgba(255,255,255,.4)", textTransform: "uppercase", fontWeight: 600, letterSpacing: "0.5px" }}>Départ</p>
                      <p style={{ margin: "2px 0 0 0", fontSize: 16, color: "#fff", fontWeight: 700 }}>{trajet.typeTrajet.toLowerCase() === 'aller' ? trajet.hay?.nom : trajet.campus?.nom}</p>
                    </div>
                    
                    <div style={{ position: "relative" }}>
                      <div style={{ position: "absolute", left: "-31px", top: "2px", width: "18px", height: "18px", borderRadius: "50%", background: "rgba(8,22,12,1)", border: "4px solid rgba(255,255,255,0.2)", zIndex: 2 }}></div>
                      <p style={{ margin: 0, fontSize: 12, color: "rgba(255,255,255,.4)", textTransform: "uppercase", fontWeight: 600, letterSpacing: "0.5px" }}>Arrivée</p>
                      <p style={{ margin: "2px 0 0 0", fontSize: 16, color: "#fff", fontWeight: 700 }}>{trajet.typeTrajet.toLowerCase() === 'aller' ? trajet.campus?.nom : trajet.hay?.nom}</p>
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    <div style={{ width: 42, height: 42, borderRadius: 14, background: "rgba(74,222,128,.15)", border: "1px solid rgba(74,222,128,.3)", display: "flex", alignItems: "center", justifyContent: "center", color: "#4ade80", fontWeight: 900, fontSize: 18 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ margin: "2px 0 0 0", fontSize: 12, color: "rgba(255,255,255,.5)", fontWeight: 500 }}>
      ...
      {modal.isOpen && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,.7)", backdropFilter: "blur(10px)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, animation: "fadeIn 0.2s ease" }}>
          <div style={{ background: "rgba(10,26,15,1)", border: "1px solid rgba(74,222,128,.3)", borderRadius: 24, padding: 32, width: "100%", maxWidth: 360, textAlign: "center", boxShadow: "0 20px 50px rgba(0,0,0,0.5)" }}>
            
            {modal.status === 'confirm' && (
              <>
                <div style={{ fontSize: 48, marginBottom: 16 }}>❓</div>
                <h3 style={{ color: "#fff", fontSize: 22, fontWeight: 900, marginBottom: 12 }}>Confirmation</h3>
                <p style={{ color: "rgba(255,255,255,.6)", fontSize: 14, marginBottom: 24, lineHeight: "1.5" }}>Êtes-vous sûr de vouloir réserver une place pour ce trajet ?</p>
            ...
            {modal.status === 'loading' && (
              <div style={{ padding: "30px 0", color: "#4ade80", fontWeight: 700, fontSize: 16, animation: "fadeIn 1s infinite alternate" }}>Enregistrement... ⏳</div>
            )}
            ...
            {modal.status === 'success' && (
              <>
                ...
                <p style={{ color: "#4ade80", fontSize: 14, marginBottom: 16 }}>{modal.message}</p>
              </>
            )}
        ...
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,.7)", backdropFilter: "blur(10px)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, animation: "fadeIn 0.2s ease" }}>
          <div style={{ background: "rgba(10,26,15,1)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 24, padding: 32, width: "100%", maxWidth: 400, boxShadow: "0 20px 50px rgba(0,0,0,0.5)", position: "relative" }}>
            ...
            <h3 style={{ color: "#fff", fontSize: 22, fontWeight: 900, marginBottom: 24, textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
              <span style={{ color: "#4ade80" }}>ℹ️</span> Détails du Trajet
            </h3>

            {/* Info Conducteur */}
            <div style={{ background: "rgba(255,255,255,.02)", border: "1px solid rgba(255,255,255,.05)", padding: 16, borderRadius: 16, marginBottom: 16 }}>
              <p style={{ color: "rgba(255,255,255,.4)", fontSize: 11, textTransform: "uppercase", fontWeight: 700, margin: "0 0 8px 0" }}>Conducteur</p>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 46, height: 46, borderRadius: "50%", background: "linear-gradient(135deg, #22c55e, #16a34a)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 900, fontSize: 20 }}>
                  {modalDetails.trajet.conducteur?.nom?.charAt(0).toUpperCase() || "?"}
                </div>
                <div>
                  <p style={{ margin: 0, fontSize: 16, fontWeight: 800, color: "#fff" }}>{modalDetails.trajet.conducteur?.nom} {modalDetails.trajet.conducteur?.prenom}</p>
                  <p style={{ margin: "2px 0 0 0", fontSize: 13, color: "#4ade80", fontWeight: 600 }}>📞 {modalDetails.trajet.conducteur?.telephone || "Non renseigné"}</p>
                </div>
              </div>
            </div>
            ...
            {/* Info Voiture */}
            <div style={{ background: "rgba(255,255,255,.02)", border: "1px solid rgba(255,255,255,.05)", padding: 16, borderRadius: 16, marginBottom: 16 }}>
              <p style={{ color: "rgba(255,255,255,.4)", fontSize: 11, textTransform: "uppercase", fontWeight: 700, margin: "0 0 8px 0" }}>Véhicule</p>
              <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: "#fff", display: "flex", alignItems: "center", gap: 8 }}>
                🚗 {modalDetails.trajet.voiture?.marque} {modalDetails.trajet.voiture?.modele}
              </p>
              <p style={{ margin: "4px 0 0 0", fontSize: 13, color: "rgba(255,255,255,.5)" }}>Couleur: {modalDetails.trajet.voiture?.couleur || "N/A"} | Immatriculation: {modalDetails.trajet.voiture?.immatriculation || "N/A"}</p>
            </div>

            {/* Info Trajet (Résumé) */}
            <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
              <div style={{ flex: 1, background: "rgba(34,197,94,.05)", border: "1px solid rgba(34,197,94,.2)", padding: 12, borderRadius: 16, textAlign: "center" }}>
                <p style={{ color: "#4ade80", fontSize: 11, textTransform: "uppercase", fontWeight: 700, margin: "0 0 4px 0" }}>Places Dispo</p>
                <p style={{ margin: 0, fontSize: 24, fontWeight: 900, color: "#fff" }}>{modalDetails.trajet.placesDisponibles}</p>
              </div>
              <div style={{ flex: 1, background: "rgba(34,197,94,.05)", border: "1px solid rgba(34,197,94,.2)", padding: 12, borderRadius: 16, textAlign: "center" }}>
                <p style={{ color: "#4ade80", fontSize: 11, textTransform: "uppercase", fontWeight: 700, margin: "0 0 4px 0" }}>Prix / Place</p>
                <p style={{ margin: 0, fontSize: 24, fontWeight: 900, color: "#fff" }}>{modalDetails.trajet.prixParPlace}<span style={{ fontSize: 12, marginLeft: 4 }}>MAD</span></p>
              </div>
            </div>
